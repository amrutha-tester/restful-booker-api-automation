...
def test_booking_lifecycle(base_url,headers,booking_payload):
    ...
    # Create booking
    create_response = requests.post(f"{base_url}/booking", json=booking_payload, headers=final_headers)
    ...
    # Get booking
    get_response = requests.get(f"{base_url}/booking/{booking_id}", headers=final_headers)
    ...
    update_response = requests.put(f"{base_url}/booking/{booking_id}", json=updated_payload, headers=final_headers)
    ...
    # Delete booking
    delete_response = requests.delete(f"{base_url}/booking/{booking_id}", headers=final_headers)
